...
export function selectAreaAsync() {
    return new Promise((resolve) => {
        ...
        // Enable transparency if available
        ...
        window.connect('draw', (widget, cr) => {
            ...
            if (data.buttonPressed && data.rect.width > 0 && data.rect.height > 0) {
                ...
                styleContext.save();
                styleContext.add_class(Gtk.STYLE_CLASS_RUBBERBAND);

                // Clear the selected area to be more visible
                cr.setOperator(cairo.Operator.SOURCE);
        ...
        window.connect('button-release-event', (widget, event) => {
            ...
            if (data.rect.width < 5 || data.rect.height < 5) {
                data.aborted = true;
            }
        ...
        window.connect('destroy', () => {
            // Small delay to let compositor redraw
